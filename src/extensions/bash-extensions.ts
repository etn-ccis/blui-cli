...
import { Framework, BuildExampleProps, Branch } from '../types';
...
module.exports = (toolbox: GluegunToolbox) => {
    ...
    const copyTemplateFilesCommand = (src: string, dest: string, framework: Framework): string => {
        const branch = EXAMPLE_BRANCHES.filter((b) => b.name === framework)[0];
        return `
            mkdir ./${dest}/.circleci
            cp -r ./${src}/README.md ./${dest}/README.md
            cp -r ./${src}/LICENSE ./${dest}/LICENSE
            cp -r ./${src}/.gitignore ./${dest}/.gitignore
            cp -r ./${src}/.editorconfig ./${dest}/.editorconfig
    ...
    const createBranchesCommand = (repository: string, folder: string, framework: Framework, update: boolean = false): string => {
        const branch = EXAMPLE_BRANCHES.filter((b) => b.name === framework)[0];
        let str = `
        ...
        str += `
            git add -A
            git commit -m "Create ${strings.startCase(branch.name)} Branch"
    ...
    const buildExampleRepository = async (props: BuildExampleProps): Promise<void> => {
        ...
        try {
            ...
            for (let i = 0; i < branches.length; i++) {
                ...
                await toolbox.system.run(`cd ${TEMP_FOLDER} && ${copyTemplateFilesCommand(TEMPLATES_FOLDER, folder, branch.name)}`, { trim: true });
