...
module.exports = (toolbox: GluegunToolbox) => {
    ...
    toolbox.countRow = (label: string, list: Array<any>, padLength: number = 15): string => {
        return `${padEnd(startCase(label), padLength, ' ')}${
            toolbox.isNull(list) ? error('NOT FOUND') : success(list.length.toString())
        }`;
    };
    ...
    toolbox.parse = async (
        query: Array<Question> = [{ question: 'Which Repository?', required: true }]
