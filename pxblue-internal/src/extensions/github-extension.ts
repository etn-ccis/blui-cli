...
module.exports = (toolbox: GluegunToolbox) => {
    ...
    const { http, print, strings, prompt, bash } = toolbox;
    ...
    const api = http.create({
        ...
        headers: {
            Accept:
                'application/vnd.github.v3+json, application/vnd.github.symmetra-preview+json, application/vnd.github.dorian-preview+json, application/vnd.github.baptiste-preview+json, application/vnd.github.luke-cage-preview+json, application/vnd.github.london-preview+json',
    ...
    async function getRepositoryBranches(
        ...
        log: boolean = false
    ): Promise<ApiResponse<Object, Object>> {
        const info: any = await api.get(
            `repos/pxblue/${repo}/branches`,
    ...
    async function repairMissingBranches(repository: string, description: string): Promise<void> {
        ...
        await bash.buildExampleRepository({
            repository,
            description,
    ...
    async function createRepository(repo: string, description: string, isPrivate: boolean = true): Promise<void> {
        ...
        if (response.status === 201) {
            ...
            if (toolbox.parameters.options.empty) {
                return;
            }
