...
module.exports = (toolbox: GluegunToolbox) => {
        ...
        try {
            ...
            for (let i = 0; i < branches.length; i++) {
                ...
                await toolbox.system.run(
                    `cd ${toolbox.strings.kebabCase(
                        repository
                    )} && git clone https://github.com/pxblue/${repository} -b ${branch.name} ${folder}`
                );
    ...
    const buildExampleRepository = async (props: BuildExampleProps): Promise<void> => {
        ...
        try {
            ...
            await toolbox.system.run(`cd ${TEMP_FOLDER} && ${prepareTemplateFilesCommand(repository, description)}`, {
                trim: true,
            });
            ...
            for (let i = 0; i < branches.length; i++) {
                ...
                if (update) {
                    spinner = toolbox.print.spin(`Cloning ${branch.name} Branch`);
                    await toolbox.system.run(
                        `cd ${TEMP_FOLDER} && git clone https://github.com/pxblue/${repository} -b ${branch.name} ${folder}`
                    );
                    spinner.succeed(`${branch.name} Project Cloned`);
                } else {
                ...
                await toolbox.system.run(
                    `cd ${TEMP_FOLDER} && ${copyTemplateFilesCommand(TEMPLATES_FOLDER, folder, branch.name, files)}`,
                    { trim: true }
                );
                ...
                if (branch.name === 'reactnative' || branch.name === 'ionic') {
                    ...
                    await toolbox.system.run(
                        `cd ${TEMP_FOLDER} && ${copySplashFilesCommand(TEMPLATES_FOLDER, folder, branch.name)}`,
                        { trim: true }
                    );
                ...
                if (push !== false) {
                    ...
                    await toolbox.system.run(createBranchesCommand(repository, folder, branch.name, update), {
                        trim: true,
                    });
    ...
    const updateExampleRepository = async (props: UpdateExampleProps): Promise<void> => {
        ...
        try {
            ...
            for (let i = 0; i < branches.length; i++) {
                ...
                await toolbox.system.run(
                    `cd ${TEMP_FOLDER}/${folder} && git checkout -b automated-update/${branch.name}-${now}`
                );
                ...
                spinner = toolbox.print.spin('Pushing Branch to GitHub');
                await toolbox.system.run(
                    createBranchesCommand(repository, folder, `automated-update/${branch.name}-${now}`, true),
                    { trim: true }
                );
                spinner.succeed(`${branch.name} Pushed to GitHub`);
                ...
                await toolbox.github.createPullRequest(
                    repository,
                    `automated-update/${branch.name}-${now}`,
                    branch.name,
                    'Updated Meta Data (Automated from CLI)',
                    '- Updated the meta files to the latest version from the example_template'
                );
